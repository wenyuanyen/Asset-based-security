...
def doWaterfall(loanPool, structuredSecurities, message=False):
    t = 1
    asset = []
    liability = []
    while loanPool.activeLoanCount(t) > 0:
        structuredSecurities.increaseTimePeriod()
        recovery = loanPool.checkDefaults(t)
        structuredSecurities.makePayments(loanPool.paymentDue(t) + recovery, loanPool.principalDue(t) + recovery)
    ...
    DIRRs = dict()
    for trancheCompleted in structuredSecurities:
        ALs[trancheCompleted.subordination] = trancheCompleted.AL()
        DIRRs[trancheCompleted.subordination] = trancheCompleted.DIRR()

    return asset, liability, structuredSecurities.reserveAccount, ALs, DIRRs
